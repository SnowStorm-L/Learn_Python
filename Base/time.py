...
# 此模块中的函数无法处理 1970 纪元年以前的日期和时间或太遥远的未来（处理极限取决于 C 函数库，对于 32 位系统来说，是 2038 年）
...
#
# Python 3.3 以后不被推荐，由于该方法依赖操作系统
# 建议使用 perf_counter() 或 process_time() 代替（一个返回系统运行时间，一个返回进程运行时间，请按照实际需求选择）
...
print('time.mktime ', time.mktime((2009, 2, 17, 17, 3, 38, 1, 48, 0)))
...
# 如果 t 未指定，将传入 time.localtime()。
# 如果元组中任何一个元素越界，将会抛出 ValueError 异常。
...
print('time.strftime ', time.strftime("时间, %d %b %Y %H:%M:%S +0000", time.gmtime()))

# time.strptime(string, format)
#
# 把一个格式化时间字符串转化为 struct_time。实际上它和 strftime() 是逆操作。

print('time.strptime ', time.strptime("30 Nov 14", "%d %b %y"))
