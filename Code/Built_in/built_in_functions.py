...
# 对于支持 round()的内置类型, 值被舍入为最接近的10到幂减去ndigits的倍数;
# 如果两个倍数相等, 则对偶数选择进行舍入(例如, round(0.5)和round(-0.5)为0, round(1.5)为2).
# 任何整数值对于ndigits (正数、零或负值) 都有效。
# 如果省略ndigits或None, 则返回值为整数。否则返回值的类型与数字相同.
